...
class AbstractShape {
    static counter = 0;
    static userId = "unknown"; // must be set before creating shapes
    id;
    borderColor;
    backgroundColor;
    constructor(borderColor = "black", backgroundColor = null, id = null // accept string now
    ) {
        if (id === null) {
...
function setupPopup(sm, canvas, canvasDomElm) {
...
// ---------------- Handlers ----------------
const createBackendHandlers = (toolsWrapper, moderationContainer, backendSync) => {
    ...
    return {
        ...
        setModerationState(isModerated) {
            if (isModerated) {
...
export function setupDrawer(canvasDomElm, toolElm, textAreaDomElm, buttonDomElm, moderationElm, canvasId, userId) {
