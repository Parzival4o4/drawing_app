...
async function loadPermissions(canvasId, currentUserId) {
    ...
    `;
        const input = addUserSection.querySelector("input");
        const btn = addUserSection.querySelector("button");
...
async function updatePermission(canvasId, targetUserId, newPerm) {
    try {
        const res = await fetch(`/api/canvas/${canvasId}/permissions`, {
            method: "POST",
