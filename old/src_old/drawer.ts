...
export function init() {
    ...
    const textAreaDomElm = document.getElementById("textarea") as HTMLTextAreaElement;
    const buttonDomElm = document.getElementById("button") as HTMLButtonElement;
    ...
    const sm: ShapeManager = {
        ...
        getShapeWithId: function (id: number): Shape {
        ...
        menuApi.createRadioOption(
            ...
            key => {
                sm.getSelectedIds().forEach(id => {
                    ...
                    sm.replaceShape(id, updated, false); // false = don't redraw yet
                });
                canvas.draw(); // single redraw at the end
            }
        )
    );
